...
test("it shows the title in the banner", () => {
  setup();
  // We expect that the title 'Hi, Iam benjaminmanjolo' is in the banner component
  expect(screen.getByText(/Hi, Iam benjaminmanjolo/i)).toBeInTheDocument();
});
...
test("can download cv when clicked on download cv button", async () => {
  ...
  // expect(downloadLink.download).toEqual('benjaminmanjolo-Resume.pdf');
  // expect(downloadLink.href).toEqual('/files/benjaminmanjolo-Resume.pdf');
